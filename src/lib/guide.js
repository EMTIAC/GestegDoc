...
export async function buildGuide(template) {
  ...
  L.push('')
  L.push('Le gabarit doit être enregistré sur le serveur. Pour injecter les données du')
  L.push('document, ajoutez le paramètre `data=<base64url JSON>` :')
  L.push('')
  L.push('```')
  L.push(`${origin}/print?template=${id}&data=<données encodées>&toolbar=0`)
  L.push('```')
  L.push('')
  L.push('> Sans `&data=`, le document affiche les valeurs par défaut du gabarit.')
  L.push('')
  L.push('### 3. API')
  L.push('')
  ...
  L.push('')
  L.push('### Comment envoyer les données ?')
  L.push('')
  L.push('Le paramètre `data=` attend un **JSON encodé en base64url** (les caractères')
  L.push('`+/=` du base64 sont remplacés par `-_` et les `=` finaux supprimés).')
  L.push('')
  L.push('En **JavaScript / navigateur** :')
  L.push('')
  L.push('```js')
  ...
  L.push('}')
  L.push('// Données pour CE gabarit (adaptez le chemin : recto.first_name, liste, etc.)')
  L.push('const data = encodeB64Url({ recto: { first_name: "Jean", last_name: "Dupont" } })')
  L.push(`const url = \`${origin}/print?template=${id}&data=\${data}&toolbar=0\``)
  L.push('window.open(url, "_blank")')
  L.push('```')
  L.push('')
  L.push('En **PHP** (équivalent) :')
  L.push('')
  L.push('```php')
  L.push('$data = [ "recto" => [ "first_name" => "Jean", "last_name" => "Dupont" ] ];')
  L.push('$enc = rtrim(strtr(base64_encode(json_encode($data)), "+/", "-_"), "=");')
  L.push(`$url = "${origin}/print?template=${id}&data=" . $enc . "&toolbar=0";`)
  L.push('```')
  L.push('')
  L.push('Ou, plus simple : laissez l’**API** encoder les données à votre place —')
  L.push(`\`POST ${origin}/api/print\` avec \`{ "templateId": "${id}", "data": $data }\``)
  L.push('(voir section 3).')
  L.push('')
  ...
  L.push('')
  L.push('> Pour l’encodage base64url des données, voir la section « Comment envoyer les')
  L.push('> données ? » ci-dessus (exemples JavaScript et PHP).')
  L.push('')
